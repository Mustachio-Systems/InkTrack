// ============================================================
// INKTRACK — App Logic v2.1
// Storage model: localStorage only. 
// Supports multiple sessions per day for multi-client artists.
// ============================================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ---------- Storage helpers ----------
    const StorageKeys = {
        accounts: "inktrack_accounts",
        session: "inktrack_session_email",
        entries: (email) => `inktrack_entries_${email}`
    };
    ...
    function getEntries(email) {
        return JSON.parse(localStorage.getItem(StorageKeys.entries(email)) || "[]");
    }
    function saveEntries(email, entries) {
        localStorage.setItem(StorageKeys.entries(email), JSON.stringify(entries));
    }
    ...
    if (signupForm) {
        signupForm.addEventListener("submit", (e) => {
            ...
            if (!name || !email || password.length < 6) {
                errorEl.textContent = "Fill in your name, email, and a password of at least 6 characters.";
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", (e) => {
            ...
            if (!account || account.password !== password) {
                errorEl.textContent = "That email and password don't match an account on this device.";
                errorEl.classList.remove("hidden");
                return;
            }
    ...
    function initDashboard(email) {
        ...
        const SUBMIT_DEFAULT_CLASS = "flex-1 font-mono text-xs uppercase tracking-widest bg-[var(--ink-red)] text-[var(--paper)] py-3 rounded hover:bg-[var(--ink-red-dim)] transition";
        const SUBMIT_EDIT_CLASS = "flex-1 font-mono text-xs uppercase tracking-widest bg-[var(--ink-teal-bright)] text-[var(--skin)] py-3 rounded hover:opacity-90 transition";
            ...
            importFileInput.addEventListener("change", (e) => {
                ...
                reader.onload = (evt) => {
                    try {
                        ...
                        const proceed = confirm(`Import ${parsed.entries.length} entries? This merges with your current data.`);
                        if (!proceed) return;

                        const current = getEntries(email);
                        // Ensure imported entries have IDs
                        parsed.entries.forEach(en => {
                            if(!en.id) en.id = Date.now().toString() + Math.random().toString(36).substr(2, 5);
                        });
                        
                        const merged = [...current, ...parsed.entries];
                        saveEntries(email, merged);
                        renderWeeklyHistory();
                        recalculateHistoricalBaseline();
                        alert("Backup imported.");
                    } catch (err) {
                        alert("Couldn't read that file. Make sure it's an InkTrack backup JSON.");
                    }
                };
        ...
        function renderStylePicker() {
            ...
            stylePickerEl.innerHTML = TATTOO_STYLES.map(style => `
                <button type="button" data-style="${style}"
                    class="style-pill ${selectedStyle === style ? 'active' : ''} font-mono text-[11px] uppercase tracking-wide px-2.5 py-1.5 rounded">
                    ${style}
                </button>
            `).join("");
        }
        ...
        window.triggerDeleteDay = function (entryId) {
            if (!confirm(`Delete this session? This can't be undone.`)) return;
            let entries = getEntries(email);
        ...
        if (deleteWeekBtn) {
            deleteWeekBtn.addEventListener("click", () => {
                ...
                let entries = getEntries(email);
                entries = entries.filter(entry => getMondayISOString(entry.date) !== activeTargetWeekStart);
                saveEntries(email, entries);
        ...
        function openWeeklyAuditModal(weekStart) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                let totalDayGross = daySessions.reduce((sum, e) => sum + e.grossGains, 0);
                ...
                if (daySessions.length > 0) {
                    dayHeader.innerHTML = `
                        ...
                        <div class="text-right font-mono text-xs">
                            ...
                            <span class="text-[var(--pencil-dim)]">${totalDayHours.toFixed(1)} hrs • $${(totalDayNet/totalDayHours).toFixed(2)}/hr</span>
                        </div>
                    `;
                    dayWrapper.appendChild(dayHeader);

                    const sessionList = document.createElement("div");
                    sessionList.className = "space-y-2 pl-4 border-l-2 border-[var(--line-bright)]";
                    ...
                    daySessions.forEach(data => {
                        ...
                        const styleLabel = data.style
                            ? `<span class="text-[10px] uppercase tracking-wide border border-[var(--ink-gold)]/40 text-[var(--ink-gold)] px-1.5 py-0.5 rounded">${data.style}</span>`
                            : `<span class="text-[10px] text-[var(--pencil-dim)]">No Style</span>`;
                        
                        const clientLabel = data.clientName 
                            ? `<span class="text-[var(--paper)] font-bold text-sm">${data.clientName}</span>`
                        ...
                        sessionEl.innerHTML = `
                            <div class="flex-1 min-w-[150px]">
                                <div class="flex items-center gap-2 mb-1">${clientLabel} ${styleLabel}</div>
                                <div class="font-mono text-[10px] text-[var(--pencil-dim)] flex gap-3">
                            ...
                            <div class="flex gap-1.5 w-full sm:w-auto justify-end">
                                <button data-edit="${data.id}" class="text-[10px] border border-[var(--line-bright)] hover:border-[var(--ink-teal-bright)] hover:text-[var(--ink-teal-bright)] px-2 py-1 rounded transition">Edit</button>
                                <button data-delete="${data.id}" class="text-[10px] border border-[var(--ink-red)]/30 text-[var(--ink-red)] hover:bg-[var(--ink-red)] hover:text-[var(--paper)] px-2 py-1 rounded transition">Delete</button>
                            </div>
                        `;
                    ...
                    dayHeader.innerHTML = `
                        ...
                        <button data-log="${currentDayStr}" class="text-[10px] border border-[var(--line-bright)] text-[var(--pencil)] hover:text-[var(--paper)] hover:border-[var(--paper)] px-2 py-1 rounded transition font-mono uppercase">+ Log Session</button>
                    `;
                    dayWrapper.appendChild(dayHeader);
                }
        ...
        function renderWeeklyHistory() {
            ...
            Array.from(uniqueWeeks).sort().reverse().forEach(weekStart => {
                ...
                row.title = "Click to view daily breakdown";

                row.innerHTML = `
                    <td class="py-3 px-3 font-mono text-[var(--pencil)] group-hover:text-[var(--ink-red)] transition">${weekRangeStr}</td>
                    <td class="py-3 px-3 font-display text-lg">$${totalGains.toFixed(2)}</td>
                    <td class="py-3 px-3 font-mono text-[var(--pencil)]">${totalHours.toFixed(1)} hrs</td>
                    <td class="py-3 px-3 font-mono text-[var(--ink-teal-bright)]">$${hourlyAvg.toFixed(2)}/hr</td>
                    <td class="py-3 px-3 font-mono text-[var(--paper)]">${sessionCount}</td>
        ...
        function recalculateHistoricalBaseline() {
            ...
            // Calculate all-time average hourly rate for stable projections
            let totalNet = 0;
            let totalHours = 0;
            ...
            // Project average daily work (assume 8 hour standard day for projection baseline)
        ...
        trackerForm.addEventListener("submit", (e) => {
            ...
            if (!selectedDate || isNaN(grossGains) || grossGains < 0) {
                formError.textContent = "Enter a valid date and a gross amount of $0 or more.";
                formError.classList.remove("hidden");
                return;
            }
            if (isNaN(hoursWorked) || hoursWorked <= 0) {
                formError.textContent = "Hours worked must be greater than 0.";
                formError.classList.remove("hidden");
                return;
            }
            ...
            if (editingId) {
                ...
                if (index > -1) {
                    entries[index] = { ...entries[index], date: selectedDate, clientName, grossGains, hoursWorked, supplySpend, style: selectedStyle };
                }
            } else {
                // Create new
                const newLog = { 
                    id: Date.now().toString(), // unique ID for multiple sessions per day
                    date: selectedDate, 
                    clientName, 
                    grossGains, 
                    hoursWorked, 
